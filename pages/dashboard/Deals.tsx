...
const STAGES = [
  ...
  { id: 'proposal', title: 'Proposal', color: 'bg-[#EAD07D]' },
  { id: 'negotiation', title: 'Negotiation', color: 'bg-purple-500' },
  { id: 'closed', title: 'Closed Won', color: 'bg-[#93C01F]' },
];
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-end gap-6 shrink-0">
         ...
         <div className="flex gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
                ...
                <input type="text" placeholder="Search deals..." className="w-full pl-10 pr-4 py-2.5 bg-white rounded-full text-sm outline-none shadow-sm focus:ring-1 focus:ring-[#EAD07D]" />
            </div>
                    ...
                    {stageDeals.map((deal) => (
                      <Link to={`/dashboard/deals/${deal.id}`} key={deal.id} className="block group">
                        <div className="bg-white p-5 rounded-[1.5rem] shadow-soft hover:shadow-card transition-all duration-300 border border-transparent hover:border-[#EAD07D]/30 relative overflow-hidden">
                           ...
                           <h4 className="text-lg font-bold text-[#1A1A1A] mb-4 leading-tight">{deal.title}</h4>
