...
class _TaxRatesPageState extends ConsumerState<TaxRatesPage> {
  ...
  List<TaxRate> _filter(List<TaxRate> items) {
    var filtered = items;
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((item) =>
          item.name.toLowerCase().contains(_searchQuery) ||
          (item.country?.toLowerCase().contains(_searchQuery) ?? false) ||
          (item.region?.toLowerCase().contains(_searchQuery) ?? false)).toList();
    }
    if (_selectedType != null) {
      filtered = filtered.where((item) => item.taxType == _selectedType).toList();
    }
    return filtered;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final asyncData = ref.watch(taxRatesProvider);
        ...
        child: Column(
          children: [
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _chip('All', _selectedType == null, isDark, () {
                    HapticFeedback.lightImpact();
                    setState(() => _selectedType = null);
                  }),
                  const SizedBox(width: 8),
                  ...TaxType.values.map((type) {
                    final label = TaxRate(id: '', name: '', rate: 0, taxType: type, createdAt: DateTime.now()).typeLabel;
                    return Padding(
                      padding: const EdgeInsets.only(right: 8),
                      child: _chip(label, _selectedType == type, isDark, () {
                        HapticFeedback.lightImpact();
                  ...
                  if (filtered.isEmpty) {
                    return IrisEmptyState(
                      ...
                      title: _searchQuery.isNotEmpty || _selectedType != null
                          ? 'No tax rates found' : 'No tax rates yet',
                      subtitle: _searchQuery.isNotEmpty
                          ? 'Try adjusting your search' : 'Configure tax rates for your products',
                  ...
                  return RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) => Padding(
                        ...
                        child: TaxRateCard(
                          taxRate: filtered[index],
                          onTap: () => context.push('/settings/tax-rates/${filtered[index].id}'),
