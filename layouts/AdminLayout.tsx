...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] text-white font-sans selection:bg-[#EAD07D] selection:text-[#1A1A1A]">
      {/* Fixed Admin Header */}
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-3 bg-[#1A1A1A] border-b border-white/10">
        {/* Logo & Admin Badge */}
        ...
        <nav className="flex items-center gap-1 bg-white/5 p-1 rounded-xl border border-white/10">
          ...
          <div className="relative group">
            <button className="px-4 py-2 rounded-lg text-sm font-medium text-[#888] hover:text-white hover:bg-white/10 transition-all flex items-center gap-2">
              More
              <ChevronDown size={14} />
            </button>
            <div className="absolute top-full right-0 mt-2 w-48 bg-[#1A1A1A] rounded-xl shadow-xl border border-white/10 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
      ...
      {/* Main Content Area */}
      <main className="pt-20 pb-12 px-6 max-w-[1920px] mx-auto">
        <Outlet />
      </main>
    </div>
  );
